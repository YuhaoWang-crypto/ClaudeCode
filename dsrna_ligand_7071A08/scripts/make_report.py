...
def _sweep_caveat(tolw, test_rank):
    """The headline in the form the data actually supports.

    The near-self weight is the one parameter the benchmark cannot bound (see
    the tolerance section). When the sweep moves the fold-change materially, the
    point estimate alone overstates what is known, so the range is put in the
    verdict rather than left for a reader who reaches the appendix.
    """
    sw = (tolw or {}).get("sensitivity_sweep", {}).get("sweep") or []
    if not sw:
        return ""
    folds = [r["fold_vs_anchor"] for r in sw]
    lo, hi = min(folds), max(folds)
    point = float(test_rank["fold_vs_ProteinA_Z"])
    if hi - lo < 0.15:
        return ""
    return (f'<div class="callout warn"><p><strong>State the headline as a range, not a point.'
            f'</strong> The {point:.2f}&times; figure depends on the near-self down-weight, which '
            f'is set by judgement and which the benchmark could not bound (see '
            f'<a href="#tolerance">the tolerance filter</a>). Across the full plausible range of '
            f'that weight the ligand moves between <strong>{lo:.2f}&times;</strong> and '
            f'<strong>{hi:.2f}&times;</strong> the Protein A benchmark. The conclusion is stable '
            f'over that interval &mdash; the ligand is above the benchmark throughout and never '
            f'approaches the positive controls &mdash; but the second decimal is not a real '
            f'number, and this ligand is more exposed to the choice than an antibody-derived one '
            f'would be, because the single core being down-weighted is its dominant epitope.</p>'
            f'</div>')
...
def main():
    ...
    prom_cal = jsn("m6_promiscuity_calibration.json")
...
<h1>HLA-DR immunogenicity of {esc(test_id)}</h1>
<p class="lede">A {len(seqs[test_id])}-residue dsRNA affinity-chromatography ligand — a
double-stranded-RNA-binding domain fused to a Protein&nbsp;A Z-domain variant —
...
<div class="toc">
<a href="#verdict">Verdict</a><a href="#domains">Domains</a><a href="#panel">Panel</a><a href="#suitability">Controls</a>
...
<div class="stat"><div class="n">{test['max_promiscuity']}<span style="font-size:17px;color:var(--muted)">/{panel['panel_size_total']}</span></div>
  ...
  <div class="s">DR molecules presenting the dominant epitope &mdash;
      {float(prom_cal['test_peak_vs_universal_sb']):.2f}&times; the
      {prom_cal['universal_epitope_ceiling_sb']}/{panel['panel_size_total']} ceiling set by the
      universal T-helper epitopes on this panel</div></div>
</div>""")
...
non-human protein of this size.</p></div>""")
    ...
    A(f"""{_sweep_caveat(tolw, test_rank)}
...
all, and for the exposure arithmetic that turns this into a dose-level call.</p>""")
    ...
    art = (dom.get("articles") or {}).get(test_id)
    if art:
        A('<h2 id="domains">Which half of the molecule carries the risk?</h2>')
        A(f"""<p>{esc(test_id)} is not one domain. It is a fusion, and the two halves have entirely
different regulatory standing, so scoring it as a single object answers the wrong question. A
whole-molecule fold-change over the Protein&nbsp;A benchmark is in part a comparison of this ligand
with <em>a piece of itself</em> — which is why the pipeline was extended with a domain attribution
step for this run rather than reporting the headline alone.</p>""")
        sc = art.get("scaffold_vs_benchmark", {})
        A(f"""<div class="grid g4">
<div class="stat"><div class="n">{art['domains'][0]['share_of_pirs']*100:.0f}%</div>
  <div class="l">of pIRS in the novel domain</div>
  <div class="s">residues {art['domains'][0]['start']}&ndash;{art['domains'][0]['end']},
      {art['domains'][0]['n_epitopes']} epitopes</div></div>
<div class="stat"><div class="n">{sc.get('identity_pct','-')}%</div>
  <div class="l">scaffold identity to the benchmark</div>
  <div class="s">residues {art['domains'][1]['start']}&ndash;{art['domains'][1]['end']} against
      {esc(sc.get('benchmark',''))}, {sc.get('n_substitutions',0)} substitutions</div></div>
<div class="stat"><div class="n">{sc.get('n_shared',0)}<span style="font-size:17px;color:var(--muted)">/{sc.get('n_scaffold_epitopes',0)}</span></div>
  <div class="l">scaffold epitopes already in the benchmark</div>
  <div class="s">i.e. already present in leachate with decades of controlled clinical exposure</div></div>
<div class="stat"><div class="n">{sc.get('novel_pirs_contribution',0):.2f}</div>
  <div class="l">pIRS from engineering-created epitopes</div>
  <div class="s">population-weighted contribution of scaffold epitopes absent from the
      benchmark</div></div>
</div>""")
        A(img("fig8_domain_attribution.png"))
        A(table(art["domains"], ["domain", "range", "length", "n_epitopes", "n_foreign",
                                 "max_promiscuity", "pirs_contribution", "share_of_pirs",
                                 "pirs_density_per_100aa"],
                ["domain", "residues", "aa", "epitopes", "non-self", "peak DR breadth",
                 "pIRS contribution", "share of pIRS", "pIRS per 100 aa of domain"], "num",
                fmt={"domain": lambda r: esc(r["domain"].replace("_", " ")),
                     "range": lambda r: f'{r["start"]}&ndash;{r["end"]}',
                     "pirs_contribution": lambda r: f'{r["pirs_contribution"]:.2f}',
                     "share_of_pirs": lambda r: f'{r["share_of_pirs"]*100:.0f}%',
                     "pirs_density_per_100aa": lambda r: f'{r["pirs_density_per_100aa"]:.2f}'}))
        nh = next((d for d in art["domains"] if d.get("nearest_human")), None)
        if nh:
            hits = nh["nearest_human"]
            A(f"""<p><strong>Is the novel domain actually foreign?</strong> Assigned in this run, not
taken on assertion. Aligned against {len(read_fasta(data_path('human_sprot.fasta'))):,} reviewed
human Swiss-Prot proteins, the closest human relatives of residues
{nh['start']}&ndash;{nh['end']} are """
              + ", ".join(
                  f"<strong>{esc(h['entry'])}</strong> ({esc(h['accession'])}, "
                  f"{h['identity_pct']:.1f}% identity over {h['aligned_length']} aa)"
                  for h in hits[:2])
              + f""". Both are double-stranded-RNA-binding proteins, which corroborates the fold
call — and both sit far below any identity at which pre-existing tolerance could be argued. The
domain is a dsRBD, and it is not a human one. <span class="muted">(Search seeded on shared 5-mers
over {nh['human_candidates_aligned']} candidate proteins and ranked on alignment score, not percent
identity; a seeded search can miss a remote homologue, so this bounds the nearest hit rather than
proving none is closer.)</span></p>""")

        if sc:
            bench_pirs = next((float(r["pIRS"]) for r in tsv("m5_ligand_summary.tsv")
                               if r["id"] == sc["benchmark"]), None)
            scaff = art["domains"][1]
            same = (bench_pirs is not None
                    and abs(scaff["pirs_density_per_100aa"] - bench_pirs) < 0.02)
            A(f"""<div class="callout {'good' if same else ''}"><p>
<strong>The scaffold half scores like the benchmark, because it essentially is the benchmark.</strong>
Per 100&nbsp;aa the Z-domain half of the ligand carries
<strong>{scaff['pirs_density_per_100aa']:.2f}</strong> pIRS units against
<strong>{bench_pirs:.2f}</strong> for {esc(sc['benchmark'])} scored standalone in the same batch"""
              + (" — the same number to two decimals." if same else ".") +
f""" {sc['n_shared']} of its {sc['n_scaffold_epitopes']} predicted epitopes overlap an epitope the
benchmark already carries, so they are not new exposure: they are the epitope content of a leachate
that has been administered to patients for decades. The {sc['n_substitutions']} substitutions that
distinguish this scaffold from the wild-type domain created
<strong>{sc['n_novel']}</strong> epitope the benchmark does not carry, contributing
<strong>{sc['novel_pirs_contribution']:.2f}</strong> to pIRS.</p></div>""")

            nov = [c for c in sc["epitopes"] if not c["shared_with_benchmark"]]
            if nov:
                A("<h3>The epitopes the engineering created</h3>")
                A(table(nov, ["core", "ligand_pos", "benchmark_pos", "best_el_rank",
                              "n_sb_alleles", "pop_presenting", "substituted_positions_in_core"],
                        ["binding core", "position in ligand", "aligned position in benchmark",
                         "best EL %Rank", "DR molecules", "population presenting",
                         "substituted residues inside the core"], "num",
                        fmt={"core": lambda r: f'<code>{esc(r["core"])}</code>',
                             "best_el_rank": lambda r: f'{r["best_el_rank"]:.2f}',
                             "pop_presenting": lambda r: f'{r["pop_presenting"]*100:.1f}%',
                             "substituted_positions_in_core":
                                 lambda r: ", ".join(str(x) for x in
                                                     r["substituted_positions_in_core"]) or "—"}))
                zero = [c for c in nov if c["pop_presenting"] == 0]
                if zero:
                    A(f"""<div class="callout warn"><p><strong>Read that
{'zero' if len(zero) == 1 else 'those zeros'} carefully.</strong>
{esc(', '.join(c['core'] for c in zero))} is predicted to be presented, but only by a DRB3/4/5
molecule — and DRB3, DRB4 and DRB5 carry no frequencies in the DRB1-locus tables the coverage model
uses, so their population weight is structurally zero. <em>It is not a statement that nobody
presents this peptide.</em> DRB5*01:01 in particular travels on the DR15 haplotype and is common in
European populations. The honest reading is that this epitope is real, narrow, and unquantifiable on
the population scale this report uses; if the scaffold's engineering is revisited, it is the one
scaffold position worth looking at.</p></div>""")
...
headline.</p>""")
    ...
    # ------------------------------------------------------------ tolerance
    A('<h2 id="tolerance">Hits a human repertoire has already seen are not risk</h2>')
    A(f"""<p>A predicted DR binder whose 9-mer core also occurs in the human proteome is a core the
repertoire has had the chance to be tolerised against. Counting those alongside genuinely foreign
ones inflates every protein that shares structure with something human — antibody-derived ligands
worst of all, since their frameworks are near-identical to human germline V. Each core here is
compared to every 9-mer of the human Swiss-Prot proteome: {val.get('real_hit_rate_9of9',0)*100:.1f}% are exact human 9-mers,
...
{'more than' if int(germ['max_promiscuity']) > int(test_rank['max_promiscuity']) else 'the same as'}
the test article's {test_rank['max_promiscuity']}/{panel['panel_size_total']}, and an unfiltered pIRS
of {float(germ['pIRS_raw']):.2f} against the test article's {float(test_rank['pIRS_raw']):.2f}. On raw
binder counts a sequence nobody can respond to is indistinguishable from one that matters. After the
filter the germline control falls to {float(germ['pIRS']):.2f} and the ranking becomes readable.</p>
<p>For this ligand the filter is a light touch rather than the whole story: it removes
<strong>{(1 - float(test_rank['pIRS']) / float(test_rank['pIRS_raw']))*100:.0f}%</strong> of the raw
score, all of it from a single core. That is the expected behaviour for a non-antibody scaffold —
there is no germline framework here for the filter to strip — and it means this ligand's ranking
rests on the prediction rather than on the filter's judgement calls.</p></div>""")
    if tolw:
...
{len(test_foreign)} peptides.</p>""")
    ...
    A(f"""<p>The test article <strong>{esc(test_id)}</strong> is sponsor-supplied and proprietary —
it has no public database identifier, and none is required: the pipeline works from de-identified
FASTA alone. Every other sequence in the batch is public and was fetched live from RCSB or UniProt
with the accession shown below, so the comparators and controls are reproducible even though the
test article is not. Domain assignments for the test article were made in this run by alignment,
not taken on assertion — see the domain attribution section above.</p>""")
    rows = []
    for r in qc:
...
Research use only — not for regulatory submission without confirmatory wet-lab data.</p></footer>""")
    ...
    html = (f"<title>{esc(test_id)} — HLA-DR immunogenicity</title>\n{FONTS}\n"
            f"<style>{CSS}</style>\n" + "\n".join(H))
