...
pres.title = `${data.test_id} HLA-DR immunogenicity`;
...
// ------------------------------------------------------------------ 1 title
{
  const s = dark("HLA-DR immunogenicity of a dsRNA affinity ligand",
    `${data.test_id} · ${data.test_length} aa · dsRNA-binding domain fused to a ` +
    `Protein A Z-domain variant\n` +
  ...
  s.addNotes("Test article is sponsor-supplied and proprietary - no public accession, and none is " +
             "needed: the pipeline works from de-identified FASTA. Every comparator and control in " +
             "the batch is public and was fetched live with its accession, so the scale the ligand " +
             "is read against is reproducible even though the ligand is not.");
}
...
// -------------------------------------------------------------- 2 exec summ
{
  ...
  const bullets = [
    ...
    [(() => {
       const a = (data.domains.articles || {})[data.test_id];
       if (!a) return "Domain attribution was not run for this article.";
       const nov = a.domains[0], sc = a.scaffold_vs_benchmark || {};
       return `The headline understates where the risk sits. ${pct(nov.share_of_pirs, 0)} of pIRS `
         + `is in the novel dsRBD (residues ${nov.start}\u2013${nov.end}); the Protein A scaffold `
         + `half scores ${num(a.domains[1].pirs_density_per_100aa)} per 100 aa against `
         + `${num(ar.pIRS)} for the benchmark itself, and ${sc.n_shared}/${sc.n_scaffold_epitopes} `
         + `of its epitopes are ones the qualified leachate already carries.`;
     })(), WARN],
...
// ------------------------------------------- 2b domain attribution
if (data.domains && (data.domains.articles || {})[data.test_id] && data.figures.domains) {
  const a = data.domains.articles[data.test_id];
  const nov = a.domains[0], scf = a.domains[1], sc = a.scaffold_vs_benchmark || {};
  const nh = (nov.nearest_human || []);
  const s = light("Which half of the molecule carries the risk?",
                  "Module 14 · domain attribution");
  s.addText("This ligand is a fusion, and the two halves have entirely different regulatory " +
            "standing. Scoring it as one object makes the headline fold-change partly a " +
            "comparison of the molecule with a piece of itself — so the risk was attributed " +
            "per domain, and the scaffold half compared epitope-by-epitope against the " +
            "benchmark it derives from.",
    { x: M, y: 1.14, w: W - 2 * M, h: 0.68, fontSize: 11, color: INK, fontFace: BODY,
      margin: 0, lineSpacing: 14 });
  s.addImage({ path: data.figures.domains, x: M, y: 1.86, w: 8.05, h: 3.14 });

  const x2 = M + 8.3, w2 = W - M - x2;
  const facts = [
    [`${pct(nov.share_of_pirs, 0)} of pIRS is in the novel domain`,
     `residues ${nov.start}–${nov.end}, ${nov.n_epitopes} epitopes, peak breadth ` +
     `${nov.max_promiscuity}/${data.panel.panel_size_total}` +
     (nh.length ? `; closest human relatives ${nh[0].entry} ${nh[0].identity_pct}% and ` +
                  `${nh[1].entry} ${nh[1].identity_pct}% — dsRBDs, but not human ones` : "")],
    [`The scaffold half is the benchmark`,
     `${sc.identity_pct}% identical to ${sc.benchmark} over ${sc.identity}; ` +
     `${num(scf.pirs_density_per_100aa)} pIRS per 100 aa against ` +
     `${num(data.anchor_rank.pIRS)} for ${sc.benchmark} scored standalone in this batch`],
    [`${sc.n_shared} of ${sc.n_scaffold_epitopes} scaffold epitopes are not new exposure`,
     `they overlap epitopes the qualified Protein A leachate already carries`],
    [`${sc.n_substitutions} substitutions created ${sc.n_novel} epitope, worth ` +
     `${num(sc.novel_pirs_contribution)} pIRS`,
     `and it is presented only by a DRB3/4/5 molecule, which carries no DRB1-locus frequency — ` +
     `so its zero population weight is an artefact of the coverage model, not an absence of risk`],
  ];
  let yy = 1.86;
  facts.forEach(([h, t]) => {
    s.addText([{ text: h + "\n", options: { bold: true, color: ACCENT } },
               { text: t, options: { color: INK } }],
      { x: x2, y: yy, w: w2, h: 1.18, fontSize: 10, fontFace: BODY, margin: 0, lineSpacing: 12.5 });
    yy += 1.22;
  });

  s.addShape(pres.ShapeType.roundRect, { x: M, y: 5.02, w: 8.05, h: 1.42,
    fill: { color: SOFT }, line: { color: LINE, width: 0.75 }, rectRadius: 0.06 });
  s.addText([
    { text: "What this changes for the risk call.  ", options: { bold: true, color: WARN } },
    { text: `The whole-molecule number is ${num(data.test_rank.fold_vs_ProteinA_Z, 2)}× the ` +
            `Protein A benchmark, but that is the wrong unit here. Essentially all of the ` +
            `elevation is the novel dsRBD, at ${num(nov.pirs_density_per_100aa)} pIRS per 100 aa ` +
            `against ${num(data.anchor_rank.pIRS)} for the benchmark — about ` +
            `${(nov.pirs_density_per_100aa / parseFloat(data.anchor_rank.pIRS)).toFixed(1)}×. ` +
            `The scaffold contributes essentially nothing new, so it is the RNA-binding domain ` +
            `that any confirmatory assay and any deimmunisation effort should target.`,
      options: {} },
  ], { x: M + 0.2, y: 5.14, w: 7.65, h: 1.2, fontSize: 10, color: INK, fontFace: BODY,
       margin: 0, lineSpacing: 12.5 });

  note(s, "Domain boundaries were assigned in this run by local alignment, not taken from the " +
          "supplied annotation; the human search is 5-mer seeded, so it bounds the nearest human " +
          "hit rather than proving none is closer.", 6.58);
  s.addNotes("The point of this slide: do not let a chimera's headline fold-change be read as a " +
             "property of the whole molecule when one domain is already a qualified affinity " +
             "ligand. Split it, and compare the shared half against the benchmark epitope by " +
             "epitope rather than by score.");
}
