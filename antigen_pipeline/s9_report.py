...
def _narrative(d: dict, lang: str) -> dict:
    ...
    if lang == "en":
        return {
            ...
            "s5": (
                ...
                f"and normal-tissue immunohistochemistry "
                f"({p['s5'].get('hpa_ihc_genes_total', p['s5']['hpa_ihc_genes']):,} genes x "
                f"{p['s5']['hpa_ihc_tissues']} tissues, of which "
                f"{p['s5']['hpa_ihc_genes']:,} survive the antibody-reliability filter). "
                f"Each arm gives an organ-weighted risk, "
    ...
    return {
        ...
        "s5": (
            ...
            f"（{p['s5'].get('hpa_ihc_genes_total', p['s5']['hpa_ihc_genes']):,} 基因 × "
            f"{p['s5']['hpa_ihc_tissues']} 组织，其中 {p['s5']['hpa_ihc_genes']:,} 个基因"
            f"通过抗体可靠性过滤）。"
