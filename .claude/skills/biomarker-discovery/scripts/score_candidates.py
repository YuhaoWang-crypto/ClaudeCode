...
# skill 自包含：默认从 ../references/ 读示例数据；可用 --candidates/--variants 覆盖
REF = os.path.join(HERE, "..", "references")
CANDIDATES = os.path.join(REF, "candidates.yaml")
VARIANTS = os.path.join(REF, "proto_variants_il23r_tyk2.tsv")
...
def main() -> None:
    ...
    ap.add_argument("--candidates", default=CANDIDATES, help="candidates.yaml 路径")
    ap.add_argument("--variants", default=VARIANTS, help="proto 变异 tsv 路径")
    args = ap.parse_args()
    ...
    print(f"✓ 载入 {len(variants)} 个待打分变异（{os.path.basename(args.variants)}）")
