...
def iedb_mhci(endpoint, method, peptides, alleles, timeout=600):
    """
    Class I prediction for a list of equal-length peptides.

    Each peptide is submitted as its own FASTA record so the tiler returns
    exactly one row per (peptide, allele) - no offset bookkeeping. Returns the
    raw IEDB columns; both heads carry 'percentile_rank', and netmhcpan_ba also
    carries 'ic50', which is the quantity a cell-free binding assay measures and
    therefore the one to compare a wet-lab result against.
    """
    peps = list(peptides)
    if not peps:
        return []
    lengths = {len(p) for p in peps}
    if len(lengths) != 1:
        raise ValueError(f"mixed peptide lengths in one call: {sorted(lengths)}")
    fasta = "".join(f">{i}\n{p}\n" for i, p in enumerate(peps))
    txt = _post(endpoint, {
        "method": method,
        "sequence_text": fasta,
        "allele": ",".join(alleles),
        "length": ",".join([str(lengths.pop())] * len(alleles)),
    }, timeout=timeout)
