...
def main():
    ...
    for allele in sorted(by_allele):
        ...
        # The skip threshold: the widest cut that still keeps MIN_SENSITIVITY of
        # real binders on the BUILD side. Widest, because skipping is the
        # irreversible move and a wider cut skips fewer things.
        ok = [o for o in ops if o["sensitivity"] >= MIN_SENSITIVITY]
        chosen = max(ok, key=lambda o: o["cut"]) if ok else max(
        ...
        rec = {
            ...
            "skip_cut_rule": (f"widest cut in {CUTS} retaining sensitivity "
                              f">= {MIN_SENSITIVITY}"),
        }
        if ba_pairs:
