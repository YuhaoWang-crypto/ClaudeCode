...
> **Track A**：在当前公开数据下，MERS-CoV Mpro/PLpro 的**计算 IC50 预测不可行**，而这个结论不是"模型没调好"，是三个可量化的硬约束造成的 —— 数据量（去重后 Mpro 仅 19 个精确 IC50）、迁移前提不成立（同一分子在 SARS 与 MERS Mpro 上效力无可检测相关性）、以及结构基础不足（两个 Mpro 受体 redock 均未过 2 Å 门槛，PLpro 根本没有可用共晶配体）。
>
