...
>
> **Track B**：virtual cell 的跨上下文迁移机制在化学扰动上**确实迁移了药物特异性的真实信号**，且**源上下文数量是主要瓶颈**：sci-Plex（3 细胞系 / 每折 2 个源）上 delta Pearson 0.239，Tahoe-100M（50 细胞系 / 每折 49 个源）上升到 **0.572**。但裸 r 会高估 —— 一个完全不含药物特异性的"通用响应"预测在 Tahoe 上就能拿到 0.348，**扣掉后药物特异性增益是 +0.224（sci-Plex 为 +0.160）**。成立条件仍然是化合物本身要产生可重复响应，这个比例从 sci-Plex 的 17.7% 升到 Tahoe 的 46.2%。
>
...
| 项 | 取值 |
|---|---|
| 覆盖（全量） | 50 细胞系 × 379 药物 × 3 浓度（0.05 / 0.5 / 5 µM），无缺口 |
| 本轮子集 | 50 细胞系 × **50 药物** × 3 浓度 = 7500 个条件 |
| 药物选取 | **强制纳入 6 个 MERS 交集药物 + 其余 44 个按固定种子随机抽取** |
| 与 MERS 数据交集 | Lopinavir、Loperamide、Ralimetinib、Gemcitabine、Benztropine、Carbidopa |

随机抽取而非"挑响应强的"，是为了避免用结果反选样本；响应强弱的筛选交给下游的可重复性门槛，与 sci-Plex 同一口径。
...
| | sci-Plex | Tahoe-100M |
|---|---|---|
| 细胞系数 / 每折源上下文数 | 3 / **2** | 50 / **49** |
| 加权共识 + gating | 0.2385 | **0.5719** |
| 负对照：通用响应 | 0.0782 | 0.3484 |
| 负对照：打乱化合物 | 0.0094 | 0.1886 |
| **药物特异性增益（vs 通用响应）** | **+0.160** | **+0.224** |
| 符号一致率 | 0.741 | **0.868** |
| top50 重叠率 | 0.056 | **0.189** |
| 可重复化合物比例 | 17.7%（99/558） | **46.2%（1151/2493）** |
| 可重复性天花板（中位） | 0.438 | 0.432 |

**四点读法**：

1. **裸 r 提升 2.4×，但扣掉通用响应后只提升 1.4×。**前者会高估源上下文数量带来的收益。
2. **两个数据集的可重复性天花板几乎相同（0.438 vs 0.432）** —— 差异不是噪声水平造成的，而是可用信号的**覆盖面**：Tahoe 有 46.2% 的 (细胞系,化合物) 对达到可重复门槛，sci-Plex 只有 17.7%，而 Tahoe 的分半定义（0.5 vs 5 µM 两个**不同剂量**）本该更难。
3. **符号一致率与 top50 重叠率的提升比 r 更可信**，因为它们不受通用响应的尺度影响：0.741→0.868、0.056→0.189。
4. 有效性判据：源侧口径下 **c1/c2/c3/c4/c5 全部通过**（两个数据集皆然）。唯一不通过的是新拆出的 **c3b（gating 是否提升相关性）**——见下。
...
| | sci-Plex | Tahoe |
|---|---|---|
| 加权 vs 未加权（同 gating） | +0.019 ✅ | +0.005 ✅ |
| gating 对 r 的影响 | 0.2385 vs 0.2556 → **−0.017** | 0.5719 vs 0.6192 → **−0.047** |
| gating 对 MAE 的影响 | 0.296 vs 0.332 → **−0.036（改善）** | 0.236 vs 0.204 → **+0.033（变差）** |
...
![B5](figures/mers/b5_tahoe_loso.png)
...
4. 本轮只取 50/379 个药物；扩到全部药物是直接的算力问题，不涉及方法变更。
...
| **更多源上下文** | 已验证为**主要瓶颈**：2 个源 → 49 个源，药物特异性增益 +0.160 → +0.224，符号一致率 0.741 → 0.868 | 本项目已用 Tahoe-100M 执行（§4.4）。再往上扩到全部 379 药物是算力问题，不涉及方法变更 |
...
| Track B 使用 sci-Plex + Tahoe 子集 | ✅ 两者均已执行并正面对比 | §4.1–4.2、§4.4 |
...
python -m mers_pipeline.b4_tahoe           # Tahoe-100M 子集提取（按 row group 选择性读取）
python -m mers_pipeline.b5_tahoe_loso      # Tahoe LOSO 基准 + 与 sci-Plex 对比
```
