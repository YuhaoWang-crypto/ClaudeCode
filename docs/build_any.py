#!/usr/bin/env python3
"""Generalized report builder: markdown -> self-contained HTML (base64-inlined imgs).
Usage: python docs/build_any.py <src.md> <out.html> "<title>"
Image srcs are resolved relative to docs/ so both figures/... and nsclc_repro/... work.
"""
import markdown, pathlib, base64, re, mimetypes, sys
...
body = re.sub(r'src="([^"]+\.(?:png|jpg|jpeg|svg|gif))"', inline, body)
...
strong{ color:#0b3d2e; }
img{ display:block; max-width:100%; height:auto; margin:12px auto 6px; border:1px solid var(--line); border-radius:8px; box-shadow:0 2px 10px rgba(0,0,0,.08); background:#fbfdff; }
...
"""
doc = f'<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{title}</title><style>{CSS}</style></head><body><div class="wrap">{body}</div></body></html>'
out_path.write_text(doc, encoding="utf-8")
print(f"{out_path.name}: {out_path.stat().st_size} bytes | imgs inlined: {doc.count('data:image')}")
