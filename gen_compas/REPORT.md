...
Both were found by the results not making sense, and both are worth recording
because anyone rebuilding this will hit them.

1. **A single idealised starting geometry is not good enough.** Building
   ...
   of nanoseconds. The seeding step now scans the whole Ramachandran plane,
   minimises every point, and takes the deepest minimum that lies inside the
   target macrostate.

2. **State definitions have to follow the actual landscape.** Defining state A
