...
### From PDB template to binding-site residues (`interface_residues.py`)

The predicted-PPI set tags each pair with PDB structural templates. This is the
bridge to the **binding-site / interface layer**: given a template, extract which
residues of each protein actually form the interface (where to design a PPI
inhibitor). `data/structure_templates_immune.tsv` lists the 68 structure-ready
predicted pairs touching the immune module with their template PDB IDs, e.g.:

| pair | RFprob | template (exact) |
|------|--------|------------------|
| PDCD1–CD274 (PD-1/PD-L1) | 0.998 | 4ZQK, 5IUS |
| TIGIT–PVR | 0.999 | 3UDW |
| FOS–JUN (AP-1) | 0.999 | 1FOS |
| IFNG–IFNGR1 | 0.998 | 1FG9, 6E3K |
| IL7–IL7R | 0.963 | 3DI3 |
| BTLA–TNFRSF14 (HVEM) | 0.998 | 2AW2 |
| PDCD1–PDCD1LG2 (PD-1/PD-L2) | 0.835 | 6UMT |

```bash
python3 interface_residues.py --list                          # the candidate table
python3 interface_residues.py --pdb 4zqk.pdb --chains A,B      # compute interface
python3 interface_residues.py --pair PDCD1 CD274               # auto-looks up 4ZQK/5IUS
```

Method: heavy-atom cross-chain contacts (≤5 Å; a tighter ≤3.5 Å pass flags the
H-bond/salt-bridge **core**); a residue is an interface residue if it has any
cross-chain contact, ranked by contact count = **interface hotspots**. Output is
a per-residue table (`results/interface_*.tsv`) plus the closest residue–residue
pairs (the binding core).

**Verified in-sandbox** on the real multi-chain structure 2BEG (the algorithm
correctly recovers the amyloid stacking core — Phe19/Phe20 hotspots, sub-2.6 Å
contacts; see `results/interface_2BEG_verification.tsv`). The checkpoint template
structures (4ZQK etc.) can't be downloaded here — RCSB/PDBe/AlphaFold are all
outside the sandbox allowlist (HTTP 403) — so the tool tries the gated fetch and,
when blocked, tells you to pass a local `--pdb` file. Add the host to egress
settings, or upload the PDB, to get the real PD-1/PD-L1 interface residues; the
tool reproduces them exactly.
...
ppi_min.py                         connectivity track (+ --predictions for real data)
interface_residues.py              PDB template → binding-site / interface residues
...
data/structure_templates_immune.tsv  structure-ready immune pairs + PDB templates
results/                           saved ranked tables + interface residue tables
