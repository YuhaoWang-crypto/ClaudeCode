...
| `t14_mediators` | PPI degree / pathway pleiotropy / gene age | only gene age absorbs anything (16.6%); degree 1.2%, pleiotropy 2.0%. ~75–80% of the organ effect is still unexplained after six controls |
...
| `t20_signalling` | signalling vs metabolic; organelle gradient | signalling is **2× SLOWER** than metabolic (0.061 vs 0.122, p=9.7e-39) despite being interface-rich. The catalytic/accessory split is a **signalling** phenomenon (1.48×) and is **absent in metabolic** (0.98×, p=0.96) — which corrects T19's reading. Rate falls from plasma membrane to nucleus (1.34×), surviving τ/expression/length **and** pathway fixed effects |
| `t21_crosscheck` | adjudicate vs an independent report | most conclusions replicate across two near-disjoint pipelines (metabolic ~2× faster, brain slowest, rank stable across depths, retention bias). Their assembly-factor result (2.79×) **corrects T20's metabolic 0.98×** as a proxy failure. Their structural "compensatory evolution" does **not** hold up: 38.8% of all pairs already clear their TM floor, pLDDT explains **7.5×** more TM variance than dN, and inside the set TM–dN is flat (ρ=−0.172, p=0.10) |
| `t22_orthologs` | ~180-species orthologue panel on human coordinates | Ensembl Compara pairwise protein alignments projected onto ungapped human positions; one2one only (one2many would mix SUCLA2 with SUCLG2 and manufacture the very signal T24 tests for) |
| `t23_control` | decisiveness by perturbation, not expression | kinetic TCA + respiratory model, capacities calibrated to measured NAD/quinone poise and turnover. Control sits on the **oxidase (0.727)** and **CS (0.143)**; **SUCL = 0.000** and survives a 200× knockdown. Holds across a 200-fold capacity-allocation sweep. Complex II never reverses but stays within **~5%** of its reversal threshold across 100× O₂. Summation theorem = 1.00002 as a unit test. Caught a Haldane error that had every Keq off by its Km ratio (MDH2 by 37×) |
| `t24_swap` | N×N subunit-swap matrix over 41 interfaces | leave-two-out PMI on structural contacts, vs **entropy-matched non-contacts** (8 draws). Coevolution is real (+0.734 excess, p=2.4e-7, 33/41) but **only 28% of the naive signal survives the control** — the rest is phylogeny. Mito-nuclear interfaces are **not** more coevolved than nuclear-nuclear (p=0.221) |
| `t25_axes` | the three-axis map, C^J as marker size | no axis predicts decisiveness: τ vs C^J ρ=**−0.273**, peak expression +0.345, all n.s. at n=10. Classic mito/cytosolic paralogues are **co-expressed, not partitioned** (GOT1/GOT2 ρ=+0.65) |
